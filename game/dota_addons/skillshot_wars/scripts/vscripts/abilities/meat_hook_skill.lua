...
function meat_hook_skill:OnAbilityPhaseStart()
...
function meat_hook_skill:OnAbilityPhaseInterrupted()
	self:GetCaster():RemoveGesture( ACT_DOTA_OVERRIDE_ABILITY_1 )
end

--------------------------------------------------------------------------------

function meat_hook_skill:OnSpellStart()
...
function meat_hook_skill:OnProjectileHit( hTarget, vLocation )
	...
	if self.bRetracting == false then
		...
		if hTarget ~= nil then
			...
			hTarget:AddNewModifier( self:GetCaster(), self, "modifier_meat_hook_skill", nil )
		...
		if self.hVictim ~= nil then
			...
			self.hVictim:InterruptMotionControllers( true )
			self.hVictim:RemoveModifierByName( "modifier_meat_hook_skill" )
...
function meat_hook_skill:OnProjectileThink( vLocation )
	self.vProjectileLocation = vLocation
end

--------------------------------------------------------------------------------

function meat_hook_skill:OnOwnerDied()
	self:GetCaster():RemoveGesture( ACT_DOTA_OVERRIDE_ABILITY_1 );
